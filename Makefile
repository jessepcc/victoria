...
## run: run locally with the in-memory store (auto-loads .env if present)
run:
	@set -a; [ -f .env ] && . ./.env || true; set +a; \
	VICTORIA_GATEWAY_INBOUND_TOKEN="$${VICTORIA_GATEWAY_INBOUND_TOKEN:-$(GATEWAY_TOKEN)}" \
	VICTORIA_ADMIN_TOKEN="$${VICTORIA_ADMIN_TOKEN:-dev-admin}" \
	go run ./cmd/victoria
