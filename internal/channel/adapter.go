...
// replyPrompt closes the packet body. When a draft reply is present it shows the
// operator the actual message Victoria would send (so there is something
// concrete to approve or correct), then the approve/correct call to action.
func replyPrompt(p domain.ReviewPacket) string {
	var b strings.Builder
	if draft := strings.TrimSpace(p.DraftReply); draft != "" {
		fmt.Fprintf(&b, "\n\n📝 Here's the draft I'd send:\n_%s_", draft)
	}
	b.WriteString("\n\n*Approved?* Y / N  — or tap an option / tell me what to change.")
	return b.String()
}
