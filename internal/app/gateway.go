...
// next message to be the correction reasoning for the given packet. The
// follow-up is resolved as a generic "wrong action" correction; use
// SetPendingFollowupAction to carry a specific tapped button through the turn.
func (g *Gateway) SetPendingFollowup(tenantID, packetID string) {
	g.SetPendingFollowupAction(tenantID, packetID, domain.ActionWrongAction)
}

// SetPendingFollowupAction is SetPendingFollowup with the correction button the
// operator tapped, so the reasoning they type next is recorded under that
// action (e.g. wrong_facts, missing_condition) rather than a generic reject.
func (g *Gateway) SetPendingFollowupAction(tenantID, packetID string, action domain.ActionButton) {
	if !action.Valid() {
		action = domain.ActionWrongAction
	}
	g.mu.Lock()
	g.pendingFollowups[tenantID] = pendingFollowup{PacketID: packetID, Action: action, StartedAt: g.clock.Now()}
	g.mu.Unlock()
}

// ConsumePendingFollowup atomically returns and clears the pending follow-up
// for the tenant if one exists and hasn't expired, along with the action button
// that opened it.
func (g *Gateway) ConsumePendingFollowup(tenantID string) (string, domain.ActionButton, bool) {
	g.mu.Lock()
	defer g.mu.Unlock()
	...
	return state.PacketID, action, true
}
