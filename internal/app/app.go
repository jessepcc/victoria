...
func (a *App) StartCase(ctx context.Context, input StartCaseInput) (domain.CaseRun, domain.ReviewPacket, error) {
	...
	decision, ruleMatched := a.evaluateDecision(now, caseRun, input.Payload, sv)
	caseRun.DecisionPointID = decision.ID
	// Consult the agent ONLY when Victoria has no learned rule for this case —
	// a genuine decision point where it must propose an action and draft for the
	// operator to correct. When a promoted rule already decides the case, run it
	// deterministically with zero LLM cost: the correction loop turns repeated
	// LLM decisions into a cheap rule, so spend falls as Victoria learns rather
	// than paying per case forever.
	var agentOut *AgentResult
	if a.agent != nil && !ruleMatched {
		if agentOut = a.runAgent(ctx, now, caseRun, decision); agentOut != nil {
			if contains(allowedActions(caseRun.WorkflowSlug), agentOut.ProposedAction) {
				decision.ProposedAction = agentOut.ProposedAction
			}
		}
	}
	artifact := a.buildArtifact(now, caseRun, decision, agentOut)
	packet := a.buildReviewPacket(now, caseRun, decision, artifact, agentOut)
...
func (a *App) ReplayCase(ctx context.Context, input ReplayInput) (domain.CaseRun, error) {
	...
	// Replay is deterministic against the pinned SkillVersion — never invoke the
	// agent here, so a re-run reproduces exactly (the rule-match bool is unused).
	decision, _ := a.evaluateDecision(now, replay, replay.InputPayload, sv)
...
func (a *App) HandleWhatsAppInbound(ctx context.Context, tenantID string, msg channel.InboundMessage) error {
	...
	// Path 0: an explicit button / list-row tap. The interactive packet's row
	// IDs ARE the ActionButton enum values, so a tap routes straight into the
	// correction loop instead of dead-lettering. "Approve" resolves the packet
	// immediately; any of the five correction buttons opens a follow-up turn so
	// the next message becomes the "what should I have done" reasoning, carried
	// under the tapped action — making all six structured correction types
	// reachable from the phone.
	if action := domain.ActionButton(strings.TrimSpace(msg.ButtonPayload)); action.Valid() {
		if action == domain.ActionApprove {
			return a.dispatchInbound(ctx, tenantID, binding, packetID, msg, action, "")
		}
		a.gateway.SetPendingFollowupAction(tenantID, packetID, action)
		a.gateway.SendNotification(ctx, tenantID, followupPromptFor(action))
		return nil
	}

	// Path 1: a pending follow-up turn — current text is the correction reason,
	// resolved under whichever action opened the follow-up (a tapped button or a
	// bare "no", which defaults to wrong_action).
	if pendingPacket, pendingAction, ok := a.gateway.ConsumePendingFollowup(tenantID); ok {
		if pendingPacket != "" {
			packetID = pendingPacket
		}
		return a.dispatchInbound(ctx, tenantID, binding, packetID, msg, pendingAction, msg.FreeText)
	}
...
func (a *App) HandleTelegramInbound(ctx context.Context, tenantID string, msg channel.InboundMessage) error {
	...
	// A tapped inline button carries its ActionButton in ButtonPayload; route it
	// straight to the correction loop rather than dead-lettering on empty text.
	if action := domain.ActionButton(strings.TrimSpace(msg.ButtonPayload)); action.Valid() {
		return a.dispatchInbound(ctx, tenantID, binding, packetID, msg, action, msg.FreeText)
	}
	intent := classifyIntent(msg.FreeText)
	switch intent.kind {
	case "approve":
...
// evaluateDecision computes the proposed action deterministically: a learned
// rule from the active SkillVersion's manifest wins; otherwise the workflow
// default. The bool return reports whether a learned rule matched — the engine
// uses it to decide whether a wired DecisionAgent may override the action (it
// may not when a correction has already been promoted into a rule; that learned
// operating logic is the whole point of the correction loop).
func (a *App) evaluateDecision(now time.Time, run domain.CaseRun, payload map[string]any, sv domain.SkillVersion) (domain.DecisionPoint, bool) {
...
// runAgent invokes the wired DecisionAgent for an unlearned sandbox case (the
// caller gates on "no rule matched"), audits the outcome, and returns the draft
// (or nil on any failure, so the caller stays on the deterministic path). It
// bounds the call with its own deadline so a slow agent cannot wedge StartCase.
func (a *App) runAgent(ctx context.Context, now time.Time, run domain.CaseRun, decision domain.DecisionPoint) *AgentResult {
	agentCtx, cancel := context.WithTimeout(ctx, agentTimeout)
	defer cancel()
	res, err := a.agent.Propose(agentCtx, AgentRequest{
		WorkflowSlug:   run.WorkflowSlug,
		DecisionType:   decision.DecisionType,
		DefaultAction:  decision.ProposedAction,
		AllowedActions: allowedActions(run.WorkflowSlug),
		Payload:        cloneMap(run.InputPayload),
	})
	if err != nil {
		_, _ = a.store.CreateAuditEvent(ctx, auditEvent(a.ids, now, run.TenantID, "agent_unavailable", "system", "agent", "case_run", run.ID, nil, map[string]any{
			"workflow_slug": run.WorkflowSlug,
			"error":         err.Error(),
		}, "agent-unavailable", run.TenantID, run.ID))
		return nil
	}
	_, _ = a.store.CreateAuditEvent(ctx, auditEvent(a.ids, now, run.TenantID, "agent_proposed", "system", "agent", "decision_point", decision.ID, map[string]any{
		"case_run_id": run.ID,
	}, map[string]any{
		"proposed_action": res.ProposedAction,
		"fact_count":      len(res.Facts),
		"has_draft":       res.DraftReply != "",
	}, "agent-proposed", run.TenantID, decision.ID))
	return &res
}

func (a *App) buildArtifact(now time.Time, run domain.CaseRun, point domain.DecisionPoint, agentOut *AgentResult) domain.Artifact {
	content := map[string]any{
		"proposed_action": point.ProposedAction,
		"workflow_slug":   run.WorkflowSlug,
	}
	// Prefer the agent's real draft reply; otherwise fall back to the
	// deterministic per-action template so the artifact preview is never empty.
	draft := draftBodyForAction(point.ProposedAction, point.AgentInput)
	if agentOut != nil {
		content["drafted_by"] = "agent"
		if agentOut.DraftReply != "" {
			draft = agentOut.DraftReply
		}
		if agentOut.Reasoning != "" {
			content["agent_reasoning"] = agentOut.Reasoning
		}
	} else {
		content["drafted_by"] = "template"
	}
	if draft != "" {
		content["draft_reply"] = draft
	}
...
func (a *App) buildReviewPacket(now time.Time, run domain.CaseRun, point domain.DecisionPoint, artifact domain.Artifact, agentOut *AgentResult) domain.ReviewPacket {
	expiresAt := now.Add(48 * time.Hour)
	// When the agent ran, show the facts it extracted, its rationale, and the
	// real draft it would send; the operator is correcting a concrete run, not a
	// payload echo. Fall back to the deterministic per-action template draft.
	facts := factsFromPayload(run.InputPayload)
	description := point.ProposedAction
	draft := draftBodyForAction(point.ProposedAction, point.AgentInput)
	if agentOut != nil {
		if len(agentOut.Facts) > 0 {
			facts = agentOut.Facts
		}
		if strings.TrimSpace(agentOut.Reasoning) != "" {
			description = agentOut.Reasoning
		}
		if strings.TrimSpace(agentOut.DraftReply) != "" {
			draft = agentOut.DraftReply
		}
	}
