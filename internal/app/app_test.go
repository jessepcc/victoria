...
// stubAgent is an in-process DecisionAgent for tests — no network.
...
func (s *stubAgent) Propose(_ context.Context, _ AgentRequest) (AgentResult, error) {
	s.calls++
	return AgentResult{Facts: s.facts, ProposedAction: s.action, DraftReply: s.draft, Reasoning: s.reason}, nil
}

// TestDecisionAgentDraftsRunAndRuleStillWins proves the agent integration:
// with no rule learned yet the agent's proposed action, facts and rationale
// drive the review packet; once a correction is promoted into a rule, that
// learned action overrides the agent — preserving the correction loop.
func TestDecisionAgentDraftsRunAndRuleStillWins(t *testing.T) {
	ctx := context.Background()
	store := memory.New()
	clock := &mutableClock{now: time.Date(2026, 4, 30, 12, 0, 0, 0, time.UTC)}
	agent := &stubAgent{
		action: "use_corporate_template", // allowed, differs from the send_quote default
		facts:  []domain.Fact{{Label: "intent", Value: "commercial enquiry", Confidence: 0.8}},
		draft:  "Hi! Thanks — I'll have our corporate team follow up shortly.",
		reason: "Looks like a commercial lead",
	}
	application := New(store, WithIDs(&sequenceIDs{}), WithClock(clock), WithAgent(agent))
	...
	_, packet, err := application.StartCase(ctx, StartCaseInput{
		...
		WorkflowSlug: "quote_drafting",
		Mode:         domain.ModeSandbox,
		Payload:      quotePayload("agent-draft"),
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if packet.PlannedAction.Type != "use_corporate_template" {
		t.Fatalf("planned action = %q, want the agent's use_corporate_template (no rule yet)", packet.PlannedAction.Type)
	}
	if packet.PlannedAction.Description != "Looks like a commercial lead" {
		t.Fatalf("description = %q, want the agent rationale", packet.PlannedAction.Description)
	}
	if len(packet.Facts) != 1 || packet.Facts[0].Label != "intent" {
		t.Fatalf("packet facts = %+v, want the agent-extracted facts", packet.Facts)
	}

	// Teach a rule (hold) via three corrections, then promote it.
	rejectCase(t, application, tenant, "one", "agent_msg_1")
	rejectCase(t, application, tenant, "two", "agent_msg_2")
	rejectCase(t, application, tenant, "three", "agent_msg_3")
	cands, err := application.ListCandidates(ctx, tenant.ID)
	if err != nil || len(cands) != 1 {
		t.Fatalf("candidates = %d (err %v), want 1", len(cands), err)
	}
	if _, _, err := application.PromoteCandidate(ctx, tenant.ID, cands[0].ID, "reviewer:alice", "ok"); err != nil {
		t.Fatal(err)
	}

	// Same payload now matches the learned rule. Two things must hold: the rule
	// wins over the agent, AND the agent is NOT called at all — a learned case
	// runs deterministically with zero LLM cost.
	callsBefore := agent.calls
	_, packet2, err := application.StartCase(ctx, StartCaseInput{
		TenantID:     tenant.ID,
		WorkflowSlug: "quote_drafting",
		Mode:         domain.ModeSandbox,
		Payload:      quotePayload("agent-after-rule"),
	})
	if err != nil {
		t.Fatal(err)
	}
	if packet2.PlannedAction.Type != "hold_and_request_more_info" {
		t.Fatalf("post-promotion action = %q, want the learned rule's hold_and_request_more_info (rule beats agent)", packet2.PlannedAction.Type)
	}
	if agent.calls != callsBefore {
		t.Fatalf("agent was called for a rule-matched case (%d → %d); a learned case must skip the LLM", callsBefore, agent.calls)
	}
}
...
// TestWhatsAppButtonTapDrivesCorrectionLoop guards the operator-UX regression:
// a tapped review-packet button (carried in ButtonPayload, no free text) must
// drive the correction loop instead of dead-lettering. A correction button
// opens a follow-up; the typed reason then forms a candidate. "Approve" resolves
// the packet directly.
func TestWhatsAppButtonTapDrivesCorrectionLoop(t *testing.T) {
	ctx := context.Background()
	application, store, _, tenant := newTestApp(t)
	if err := application.AcknowledgeWhatsAppConsent(ctx, tenant.ID, AcknowledgeWhatsAppConsentInput{
		InboundMode: domain.InboundModeReadOnly,
	...
	if _, _, err := application.StartCase(ctx, StartCaseInput{
		...
		WorkflowSlug: "quote_drafting",
		Mode:         domain.ModeSandbox,
		Payload:      quotePayload("button-tap"),
	}); err != nil {
		t.Fatal(err)
	}

	// Operator TAPS "Wrong action" on their own device (IsFromMe), no free text.
	if err := application.HandleWhatsAppInbound(ctx, tenant.ID, channel.InboundMessage{
		IsFromMe:          true,
		SenderIdentifier:  "61400000000",
		ProviderMessageID: "tap-1",
		Channel:           channel.ChannelWhatsApp,
		ButtonPayload:     string(domain.ActionWrongAction),
	}); err != nil {
		t.Fatal(err)
	}
	events, _ := store.ListAuditEvents(ctx, tenant.ID)
	if countEvents(events, "correction_dead_lettered") != 0 {
		t.Fatalf("a tapped button dead-lettered: %d", countEvents(events, "correction_dead_lettered"))
	}
	if cands, _ := application.ListCandidates(ctx, tenant.ID); len(cands) != 0 {
		t.Fatalf("candidate formed before the reason was typed: %d", len(cands))
	}

	// The follow-up reason now becomes the correction.
	if err := application.HandleWhatsAppInbound(ctx, tenant.ID, channel.InboundMessage{
		IsFromMe:          true,
		SenderIdentifier:  "61400000000",
		ProviderMessageID: "reason-1",
		Channel:           channel.ChannelWhatsApp,
		FreeText:          "should have held and asked for more photos first",
	}); err != nil {
		t.Fatal(err)
	}
	cands, err := application.ListCandidates(ctx, tenant.ID)
	...
	if len(cands) != 1 {
		t.Fatalf("tapped correction did not form a candidate: got %d", len(cands))
	}
	if cands[0].RecommendedAction != "hold_and_request_more_info" {
		t.Fatalf("candidate action = %q, want hold_and_request_more_info", cands[0].RecommendedAction)
	}

	// A tapped "Approve" resolves without dead-lettering.
	if _, _, err := application.StartCase(ctx, StartCaseInput{
		TenantID:     tenant.ID,
		WorkflowSlug: "quote_drafting",
		Mode:         domain.ModeSandbox,
		Payload:      quotePayload("button-approve"),
	}); err != nil {
		t.Fatal(err)
	}
	if err := application.HandleWhatsAppInbound(ctx, tenant.ID, channel.InboundMessage{
		IsFromMe:          true,
		SenderIdentifier:  "61400000000",
		ProviderMessageID: "tap-approve",
		Channel:           channel.ChannelWhatsApp,
		ButtonPayload:     string(domain.ActionApprove),
	}); err != nil {
		t.Fatal(err)
	}
	events, _ = store.ListAuditEvents(ctx, tenant.ID)
	if countEvents(events, "correction_dead_lettered") != 0 {
		t.Fatalf("approve tap dead-lettered: %d", countEvents(events, "correction_dead_lettered"))
	}
}
