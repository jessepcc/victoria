...
import (
	...
	"github.com/jessepcc/victoria/internal/agent/deepseek"
...
func main() {
	...
	// Wire a real reasoning agent (DeepSeek) when configured; otherwise the
	// engine drafts deterministically. Nil-check before WithAgent: a typed-nil
	// interface value would defeat the fallback.
	var appOpts []app.Option
	if agent := deepseek.NewFromEnv(); agent != nil {
		appOpts = append(appOpts, app.WithAgent(agent))
		log.Printf("decision agent: deepseek model=%s (set VICTORIA_AGENT_MODEL to override)", agent.Model())
	} else {
		log.Print("decision agent: disabled (set DEEPSEEK_API_KEY to enable); using deterministic drafts")
	}
	application := app.New(store, appOpts...)
